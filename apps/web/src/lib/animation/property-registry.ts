...
> = {
	...
	"transform.positionZ": createNumberPropertyDefinition({
		numericRange: { step: 1 },
		supportsElement: ({ element }) => isVisualElement(element),
		getValue: ({ element }) =>
			isVisualElement(element)
				? (element.transform.positionZ ??
					DEFAULTS.element.transform.positionZ ??
					0)
				: null,
		setValue: ({ element, value }) =>
			isVisualElement(element)
	...
	pan: createNumberPropertyDefinition({
		numericRange: { min: -100, max: 100, step: 1 },
		supportsElement: ({ element }) => canElementHaveAudio(element),
		getValue: ({ element }) =>
			canElementHaveAudio(element) ? (element.pan ?? 0) : null,
		setValue: ({ element, value }) =>
			canElementHaveAudio(element)
				? { ...element, pan: value as number }
				: element,
	}),
	color: {
		kind: "color",
