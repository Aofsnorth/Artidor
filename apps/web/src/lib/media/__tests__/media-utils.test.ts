...
describe("getMediaTypeFromFile", () => {
	test("detects audio by MIME type", () => {
		...
		expect(
			getMediaTypeFromFile({
				file: new File([], "x.flac", { type: "audio/flac" }),
			}),
		).toBe("audio");
	});
	...
	test("returns null for unrecognized files", () => {
		expect(
			getMediaTypeFromFile({ file: new File([], "x.xyz", { type: "" }) }),
		).toBeNull();
	});
});
...
describe("mediaSupportsAudio", () => {
	test("audio and video assets support audio", () => {
		expect(mediaSupportsAudio({ media: { type: "audio" } as const })).toBe(
			true,
		);
		expect(mediaSupportsAudio({ media: { type: "video" } as const })).toBe(
			true,
		);
		expect(mediaSupportsAudio({ media: { type: "image" } as const })).toBe(
			false,
		);
