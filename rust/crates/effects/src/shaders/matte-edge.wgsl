...
fn fragment_main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let threshold = 0.03 + amount * 0.3;
    let matte = smoothstep(threshold - 0.01, threshold + 0.01, edge);
    let matte_color = vec3<f32>(0.15, 0.12, 0.1);
    return vec4<f32>(mix(color.rgb, matte_color, matte), color.a);
}
